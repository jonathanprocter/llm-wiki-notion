# LLM Wiki — Schema for Technical Writer (Notion Edition)
...
**Notion integration note:** After each session, the user runs `python notion_sync.py` to push all wiki pages to their Notion workspace. You do not need to interact with Notion directly — just maintain the local Markdown files and the sync script handles the rest.
...
> **Notion note:** `[[wiki-page-name]]` links are converted to inline code by the sync script. Use them freely for cross-referencing — they remain fully readable in both local Markdown and Notion.
...
When the user says "sync to Notion" or after any ingest session:
...
This pushes all local wiki pages to their Notion workspace. The script handles creation, updates, and the `[[wiki-link]]` conversion automatically.
...
- After each session, remind the user to run `python notion_sync.py` to push changes to Notion
