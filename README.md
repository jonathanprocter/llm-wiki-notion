# LLM Wiki (Karpathy Pattern) — Notion Edition
...
- [Python 3.9+](https://www.python.org/) for running the Notion sync script
- A [Notion](https://notion.so/) account (free tier works)
- A Notion Integration token (see **NOTION_SETUP.md** for step-by-step instructions)
...
```bash
git clone https://github.com/jonathanprocter/llm-wiki-notion.git
cd llm-wiki-notion
```
...
A single source can touch 5-15 wiki pages. 
### 6. Ask questions
...
│
└── notion_sync.py         # Sync script: local wiki to Notion
...
**Sync to Notion after each session.** Run `python notion_sync.py` at the end of every work session to keep Notion up to date.
...
- Original implementation and article by [Balu Kosuri](https://github.com/balukosuri)
- Notion adaptation by [jonathanprocter](https://github.com/jonathanprocter)
